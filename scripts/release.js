...
function determineNextVersion() {
    ...
    const reverts = parsedCommits.filter((e) => e.type === 'revert');

    const relevantCommits = parsedCommits.filter(
        (commit) =>
            !reverts.find((revert) => revert.body.includes(commit.hash) || revert.subject.includes(commit.subject)),
    );

    if (relevantCommits.find((e) => e.isBreaking)) {
        console.log(semver.inc(lastReleaseOnBranch, 'major'));
        return;
    }

    if (relevantCommits.find((e) => ['feat'].includes(e.type))) {
